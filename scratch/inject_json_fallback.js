...
const injectionCode = `    // Read from the dynamically generated massive JSON file
    let questionPool = [];
    try {
      const fs = require('fs');
...
const startIndex = content.indexOf('    const questionPool = [');
const endIndex = content.indexOf('    // Shuffle the array using Fisher-Yates and pick the first 5');

if (startIndex !== -1 && endIndex !== -1) {
  content = content.substring(0, startIndex) + injectionCode + "\n\n" + content.substring(endIndex);
